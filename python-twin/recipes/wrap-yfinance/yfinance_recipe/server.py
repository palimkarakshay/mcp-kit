"""Build the yfinance recipe server: identity + tool registry.
...
SERVER_NAME = "mcp-recipe-yfinance"

_INSTRUCTIONS = (
    "Wraps yfinance (Yahoo Finance) as one MCP server: get_ticker (quote), get_news (headlines), "
    "get_chart (OHLCV history). Adds an in-memory cache and rate-limit handling so consumers don't "
    "embed yfinance directly. No credentials are required."
)


def build_server(http: HttpConfig | None = None) -> FastMCP:
    """Construct a fresh server with the yfinance recipe tools registered."""
